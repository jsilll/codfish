...
namespace codfish::core::magics {
/// @brief Pre-computed magic numbers for bishops.
static constexpr bitboard::Bitboard
    kBishopMagics[static_cast<std::size_t>(Square::Total)] = {
...
static constexpr bitboard::Bitboard
    kRookMagics[static_cast<std::size_t>(Square::Total)] = {
...
/// @note The cod::core::magics::Init() function must be called before using this.
Magic kRookTable[static_cast<std::size_t>(Square::Total)];

/// @brief Array of pre-computed magic numbers, masks and shifts for bishops.
/// @note The cod::core::magics::Init() function must be called before using this.
Magic kBishopTable[static_cast<std::size_t>(Square::Total)];
...
BishopXrayAttacks(const Square sq) noexcept {
    ...
    for (int r = rank + 1, f = file + 1;
         r < static_cast<int>(Rank::R8) and f < static_cast<int>(File::FH);
         ++r, ++f) {
    ...
    for (int r = rank + 1, f = file - 1;
         r < static_cast<int>(Rank::R8) and f > static_cast<int>(File::FA);
         ++r, --f) {
        attacks |= (bitboard::kOne << static_cast<int>(utils::GetSquare(
                        static_cast<Rank>(r), static_cast<File>(f))));
    }
    ...
    for (int r = rank - 1, f = file + 1;
         r > static_cast<int>(Rank::R1) and f < static_cast<int>(File::FH);
         --r, ++f) {
        attacks |= (bitboard::kOne << static_cast<int>(utils::GetSquare(
                        static_cast<Rank>(r), static_cast<File>(f))));
    }
...
Init() noexcept {
    for (int sq = static_cast<int>(Square::A1);
        ...
        kBishopTable[sq].mask = BishopXrayAttacks(static_cast<Square>(sq));
        kBishopTable[sq].magic = kBishopMagics[sq];
        kBishopTable[sq].shift =
            static_cast<int>(Square::Total) - utils::kBishopRelevantBits[sq];
    }

    for (int sq = static_cast<int>(Square::A1);
         sq < static_cast<int>(Square::Total); ++sq) {
        kRookTable[sq].mask = RookXrayAttacks(static_cast<Square>(sq));
        kRookTable[sq].magic = kRookMagics[sq];
        kRookTable[sq].shift =
            static_cast<int>(Square::Total) - utils::kRookRelevantBits[sq];
    }
}
}   // namespace codfish::core::magics