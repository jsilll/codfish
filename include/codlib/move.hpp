...
public:
    ...
    [[maybe_unused]] [[nodiscard]] Move(int source_square, const int target_square, const int piece,
                                        const int captured_piece, const int promoted_piece, const bool is_double_push,
                                        const bool is_en_passant, const bool is_castle) noexcept: _encoded(
            static_cast<std::uint32_t>(source_square) | (static_cast<std::uint32_t>(target_square << 6)) |
            (static_cast<std::uint32_t>(piece << 12)) | (static_cast<std::uint32_t>(captured_piece << 15)) |
            (static_cast<std::uint32_t>(promoted_piece << 18)) | (static_cast<std::uint32_t>(is_double_push << 21)) |
    ...
    [[maybe_unused]] [[nodiscard]] constexpr auto CapturedPiece() const noexcept {
        return static_cast<PieceType>((_encoded & 0x38000) >> 15);
    }

    /// @brief Returns the promoted piece
    /// @return The promoted piece
    [[maybe_unused]] [[nodiscard]] constexpr auto PromotedPiece() const noexcept {
        return static_cast<PieceType>((_encoded & 0x1C0000) >> 18);
    }

    /// @brief Returns if it is a double push
    /// @return If it is a double push
    [[maybe_unused]] [[nodiscard]] constexpr bool IsDoublePush() const noexcept { return (_encoded & 0x200000); }

    /// @brief Returns if it is an en passant
    /// @return If it is an en passant
    [[maybe_unused]] [[nodiscard]] constexpr bool IsEnPassant() const noexcept { return (_encoded & 0x400000); }

    /// @brief Returns if it is a castle
    /// @return If it is a castle
    [[maybe_unused]] [[nodiscard]] constexpr bool IsCastle() const noexcept { return (_encoded & 0x800000); }
    ...
    [[maybe_unused]] [[nodiscard]] constexpr auto encoded() const noexcept { return _encoded; }
