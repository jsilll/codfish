...
private:
  ...
  [[nodiscard]] static constexpr auto get_opponent(const Color to_move) noexcept
  {
  ...
  /// @param fl The file
  /// @return The square
  [[nodiscard]] static constexpr auto get_square(const Rank rk, const File fl) noexcept
  {
...
public:
  ...
  {
    set_from_fen(piece_placements, active_color, castling_rights, en_passant, halfmove_clock, fullmove_number);
  }
  ...
  [[nodiscard]] constexpr auto get_piece_from_square(const Square sq) const noexcept { return _square[sq]; }
  ...
  /// @param type The type
  [[nodiscard]] constexpr auto get_pieces(const Color color, const PieceType type) const noexcept
  {
  ...
  [[nodiscard]] constexpr auto get_occupancies(const Color color) const noexcept { return _occupancies[color]; }
  ...
  [[maybe_unused]] void set_en_passant_square(const Square sq) noexcept { _en_passant_square = sq; }

  /// @brief Sets the current castling rights
  /// @param castling_rights The castling rights
  [[maybe_unused]] void set_castling_rights(const int castling_rights) noexcept { _castling_rights = castling_rights; }
...
private:
  ...
  std::array<Piece, N_SQUARES> _square{};
