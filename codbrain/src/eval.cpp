...
/// @brief Piece square table for knight during middlegame
...
/// @brief Piece square table for bishop during middlegame
...
/// @brief Piece square table for rook during middlegame
...
/// @brief Piece square table for queen during middlegame
...
/// @brief Piece square table for king during middlegame
...
/// @brief Piece square table for pawn during endgame
...
/// @brief Piece square table for knight during endgame
...
/// @brief Piece square table for bishop during endgame
...
/// @brief Piece square table for rook during endgame
...
/// @brief Piece square table for queen during endgame
...
/// @brief Piece square table for king during endgame
...
/// @brief Aggregated piece square tables for middle game
static int MG_TABLE[N_COLORS][N_PIECES][N_SQUARES];

/// @brief Aggregated piece square tables for end game
static int EG_TABLE[N_COLORS][N_PIECES][N_SQUARES];

/// @brief Game phase increment for each piece type
static constexpr int GAME_PHASE_INC[N_PIECES] = {0, 1, 1, 2, 4, 0};

/// @brief Piece scores for each piece type
...
Init() noexcept {
    for (int sq = A1; sq < N_SQUARES; sq++) {
        ...
        MG_TABLE[WHITE][PAWN][sq] = MG_PAWN_TABLE[flipd];
        MG_TABLE[WHITE][KNIGHT][sq] = MG_KNIGHT_TABLE[flipd];
        MG_TABLE[WHITE][BISHOP][sq] = MG_BISHOP_TABLE[flipd];
        MG_TABLE[WHITE][ROOK][sq] = MG_ROOK_TABLE[flipd];
        MG_TABLE[WHITE][QUEEN][sq] = MG_QUEEN_TABLE[flipd];
        MG_TABLE[WHITE][KING][sq] = MG_KING_TABLE[flipd];
        ...
        EG_TABLE[WHITE][PAWN][sq] = EG_PAWN_TABLE[flipd];
        EG_TABLE[WHITE][KNIGHT][sq] = EG_KNIGHT_TABLE[flipd];
        EG_TABLE[WHITE][BISHOP][sq] = EG_BISHOP_TABLE[flipd];
        EG_TABLE[WHITE][ROOK][sq] = EG_ROOK_TABLE[flipd];
        EG_TABLE[WHITE][QUEEN][sq] = EG_QUEEN_TABLE[flipd];
        EG_TABLE[WHITE][KING][sq] = EG_KING_TABLE[flipd];
...
Static(const Board &board) noexcept {
    ...
    for (int piece_type = PAWN; piece_type < N_PIECES; piece_type++) {
        ...
        while (pieces_black) {
            const auto sq = bitboard::BitScanForward(pieces_black);
    ...
    const auto to_move = board.active();
    const auto opponent = board.inactive();

    const auto mg_score = mg[to_move] - mg[opponent];
    const auto eg_score = eg[to_move] - eg[opponent];
    const auto material_score = material[to_move] - material[opponent];
    const auto mg_phase = game_phase > 24 ? 24 : game_phase;
