...
namespace bitboard {
    ...
    [[maybe_unused]] [[nodiscard]] constexpr Square bit_scan(u64 bb) noexcept {
        if (bb) {
    ...
    [[nodiscard]] constexpr u64 sout_one(u64 bb) noexcept { return (bb >> Direction::NORTH); }

    /// @brief Shifts a bitboard one square north
    /// @param bb The bitboard
    /// @return The shifted bitboard
    [[nodiscard]] constexpr u64 nort_one(u64 bb) noexcept { return (bb << Direction::NORTH); }

    /// @brief Shifts a bitboard one square east
    /// @param bb The bitboard
    /// @return The shifted bitboard
    [[nodiscard]] constexpr u64 east_one(u64 bb) noexcept { return (bb << Direction::EAST) & 0xFEFEFEFEFEFEFEFE; }

    /// @brief Shifts a bitboard one square west
    /// @param bb The bitboard
    /// @return The shifted bitboard
    [[nodiscard]] constexpr u64 west_one(u64 bb) noexcept { return (bb >> Direction::EAST) & 0x7F7F7F7F7F7F7F7F; }

    /// @brief Shifts a bitboard one square north east
    /// @param bb The bitboard
    /// @return The shifted bitboard
    [[nodiscard]] constexpr u64 no_ea_one(u64 bb) noexcept { return (bb << Direction::NORTH_EAST) & 0xFEFEFEFEFEFEFEFE; }

    /// @brief Shifts a bitboard one square north west
    /// @param bb The bitboard
    /// @return The shifted bitboard
    [[nodiscard]] constexpr u64 no_we_one(u64 bb) noexcept { return (bb << Direction::NORTH_WEST) & 0x7F7F7F7F7F7F7F7F; }

    /// @brief Shifts a bitboard one square south east
    /// @param bb The bitboard
    /// @return The shifted bitboard
    [[nodiscard]] constexpr u64 so_ea_one(u64 bb) noexcept { return (bb >> Direction::NORTH_WEST) & 0xFEFEFEFEFEFEFEFE; }

    /// @brief Shifts a bitboard one square south west
    /// @param bb The bitboard
    /// @return The shifted bitboard
    [[nodiscard]] constexpr u64 so_we_one(u64 bb) noexcept { return (bb >> Direction::NORTH_EAST) & 0x7F7F7F7F7F7F7F7F; }
