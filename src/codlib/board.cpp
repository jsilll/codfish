...
bool Board::is_square_attacked(const Square sq, const Color attacker) const noexcept
{
...
void Board::make(const Move move) noexcept
{
  ...
  if (_to_move == BLACK) { _full_move_number++; }
...
void Board::unmake(const Move move, const GameState state) noexcept
{
  ...
  if (is_promotion) { bitboard::pop_bit(_pieces[_to_move][promoted_piece], to_square); }
  ...
  const std::string &full_move_number) noexcept
{
  for (int i = 0; i < Square::N_SQUARES; ++i) {
    _square[i].color = BOTH;
    _square[i].type = EMPTY_PIECE;
  }
...
{
  if (b1.get_side_to_move() != b2.get_side_to_move()) { return false; }
  if (b1.get_castling_rights() != b2.get_castling_rights()) { return false; }
  if (b1.get_en_passant_square() != b2.get_en_passant_square()) { return false; }
  if (b1.get_half_move_clock() != b2.get_half_move_clock()) { return false; }
  if (b1.get_full_move_number() != b2.get_full_move_number()) { return false; }
  if (b1.is_ascii() != b2.is_ascii()) { return false; }
  if (b1.is_white_on_bottom() != b2.is_white_on_bottom()) { return false; }
  ...
  for (int side = 0; side < N_SIDES + 1; side++) {
    if (b1.get_occupancies(static_cast<Color>(side)) != b2.get_occupancies(static_cast<Color>(side))) { return false; }
  }
