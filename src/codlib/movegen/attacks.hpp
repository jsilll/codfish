...
namespace attacks {
    ...
    extern bitboard::Bitboard KING_ATTACKS[Square::N_SQUARES];

    /// @brief Bitboard masks for each knight attack
    /// @note The Init() function must be called before using this
    extern bitboard::Bitboard KNIGHT_ATTACKS[Square::N_SQUARES];

    /// @brief Bitboard masks for each pawn attack
    /// @note The Init() function must be called before using this
    extern bitboard::Bitboard PAWN_ATTACKS[Color::N_COLORS][Square::N_SQUARES];
    ...
    [[nodiscard]] constexpr bitboard::Bitboard PawnAttacks(const bitboard::Bitboard pawns) noexcept {
        static_assert(C == Color::WHITE || C == Color::BLACK);
        static_assert(D == bitboard::Direction::WEST || D == bitboard::Direction::EAST);
    ...
    template<Color C>
    [[nodiscard]] constexpr bitboard::Bitboard PawnAllAttacks(const bitboard::Bitboard pawns) noexcept {
        static_assert(C == Color::WHITE || C == Color::BLACK);
        return PawnAttacks<C, bitboard::Direction::WEST>(pawns) | PawnAttacks<C, bitboard::Direction::EAST>(pawns);
    }
    ...
    PawnSinglePushes(const bitboard::Bitboard pawns, const bitboard::Bitboard empty) noexcept {
        static_assert(C == Color::WHITE || C == Color::BLACK);
    ...
    template<Color C>
    [[nodiscard]] constexpr bitboard::Bitboard
    PawnDoublePushes(const bitboard::Bitboard pawns, const bitboard::Bitboard empty) noexcept {
        static_assert(C == Color::WHITE || C == Color::BLACK);

        if constexpr (C == Color::WHITE) {
            bitboard::Bitboard single_pushes = PawnSinglePushes<Color::WHITE>(pawns, empty);
            return bitboard::ShiftOne<bitboard::Direction::NORTH>(single_pushes) & empty & utils::MASK_RANK[RANK_3];
        } else {
            bitboard::Bitboard single_pushes = PawnSinglePushes<Color::BLACK>(pawns, empty);
            return bitboard::ShiftOne<bitboard::Direction::SOUTH>(single_pushes) & empty & utils::MASK_RANK[RANK_4];
        }
    }
    ...
    [[nodiscard]] bitboard::Bitboard BishopAttacks(Square sq, bitboard::Bitboard block) noexcept;

    /// @brief Gets the rook attacks for a given square and occupancy
    /// @param sq The square
    /// @param block The occupancy
    /// @return The rook attacks
    [[nodiscard]] bitboard::Bitboard RookAttacks(Square sq, bitboard::Bitboard block) noexcept;

    /// @brief Gets the queen attacks for a given square and occupancy
    /// @param sq The square
    /// @param occ The occupancy
    /// @return The queen attacks
    [[nodiscard]] inline bitboard::Bitboard QueenAttacks(const Square sq, const bitboard::Bitboard occ) noexcept {
