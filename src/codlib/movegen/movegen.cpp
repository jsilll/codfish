...
template<Color ToMove> static void generate_castling_moves(std::vector<Move> &move_list, const Board &board) noexcept
{
  ...
  if (!board.IsSquareAttacked(CASTLE_E_SQ, Opponent)) {
    if ((board.castling_availability() & CASTLE_KING_MASK) && !bitboard::get_bit(board.occupancies(BOTH), CASTLE_F_SQ)
        && !bitboard::get_bit(board.occupancies(BOTH), CASTLE_G_SQ)) {
    ...
    if ((board.castling_availability() & CASTLE_QUEEN_MASK)
        && !bitboard::get_bit(board.occupancies(BOTH), CASTLE_D_SQ)
        && !bitboard::get_bit(board.occupancies(BOTH), CASTLE_C_SQ)
        && !bitboard::get_bit(board.occupancies(BOTH), CASTLE_B_SQ)) {
  ...
  while (pawns_can_capture_with_promo) {
    ...
    while (pawn_captures_promo) {
      ...
      move_list.emplace_back(
              from_square, to_square, PAWN, board.piece(to_square).type, QUEEN, false, false, false);
      move_list.emplace_back(
              from_square, to_square, PAWN, board.piece(to_square).type, KNIGHT, false, false, false);
      move_list.emplace_back(
              from_square, to_square, PAWN, board.piece(to_square).type, ROOK, false, false, false);
      move_list.emplace_back(
              from_square, to_square, PAWN, board.piece(to_square).type, BISHOP, false, false, false);
...
static void generate_pawn_captures_no_promotion(std::vector<Move> &move_list, const Board &board) noexcept
{
  ...
  while (pawns_can_capture_no_promo) {
    ...
    while (pawn_captures_no_promo) {
      ...
      move_list.emplace_back(
              from_square, to_square, PAWN, board.piece(to_square).type, EMPTY_PIECE, false, false, false);
  ...
  while (to_move_pieces) {
    ...
    while (moves) {
      ...
      if constexpr (GType == QUIETS) {
        ...
        move_list.emplace_back(
                from_square, to_square, PType, board.piece(to_square).type, EMPTY_PIECE, false, false, false);
      }
