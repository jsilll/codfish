...
template<Color ToMove> static void generate_castling_moves(std::vector<Move> &move_list, const Board &board) noexcept
{
  ...
  if (!board.is_square_attacked(CASTLE_E_SQ, Opponent)) {
    if ((board.get_castling_rights() & CASTLE_KING_MASK) && !bitboard::get_bit(board.get_occupancies(BOTH), CASTLE_F_SQ)
        && !bitboard::get_bit(board.get_occupancies(BOTH), CASTLE_G_SQ)) {
    ...
    if ((board.get_castling_rights() & CASTLE_QUEEN_MASK)
        && !bitboard::get_bit(board.get_occupancies(BOTH), CASTLE_D_SQ)
        && !bitboard::get_bit(board.get_occupancies(BOTH), CASTLE_C_SQ)
        && !bitboard::get_bit(board.get_occupancies(BOTH), CASTLE_B_SQ)) {
...
static void generate_en_passant_capture(std::vector<Move> &move_list, const Board &board) noexcept
{
  ...
  if (board.get_en_passant_square() != -1) {
    u64 pawns_can_en_passant =
      attacks::ATTACKS_PAWN[Opponent][board.get_en_passant_square()] & board.get_pieces(ToMove, PAWN);
    while (pawns_can_en_passant) {
      int from_square = bitboard::bit_scan_forward(pawns_can_en_passant);
      move_list.emplace_back(from_square,
        board.get_en_passant_square(),
        PAWN,
        board.get_piece_from_square(board.get_en_passant_square()).type,
        EMPTY_PIECE,
        false,
        true,
        false);
  ...
  while (pawns_can_capture_with_promo) {
    ...
    while (pawn_captures_promo) {
      ...
      move_list.emplace_back(
        from_square, to_square, PAWN, board.get_piece_from_square(to_square).type, QUEEN, false, false, false);
      move_list.emplace_back(
        from_square, to_square, PAWN, board.get_piece_from_square(to_square).type, KNIGHT, false, false, false);
      move_list.emplace_back(
        from_square, to_square, PAWN, board.get_piece_from_square(to_square).type, ROOK, false, false, false);
      move_list.emplace_back(
        from_square, to_square, PAWN, board.get_piece_from_square(to_square).type, BISHOP, false, false, false);
...
static void generate_pawn_captures_no_promotion(std::vector<Move> &move_list, const Board &board) noexcept
{
  ...
  while (pawns_can_capture_no_promo) {
    ...
    while (pawn_captures_no_promo) {
      ...
      move_list.emplace_back(
        from_square, to_square, PAWN, board.get_piece_from_square(to_square).type, EMPTY_PIECE, false, false, false);
  ...
  while (to_move_pieces) {
    ...
    while (moves) {
      ...
      if constexpr (GType == QUIETS) {
        ...
        move_list.emplace_back(
          from_square, to_square, PType, board.get_piece_from_square(to_square).type, EMPTY_PIECE, false, false, false);
      }
...
[[maybe_unused]] std::vector<Move> generate_pseudo_legal_moves(const Board &board) noexcept
{
...
[[maybe_unused]] std::vector<Move> generate_pseudo_legal_captures(const Board &board) noexcept
{
  std::vector<Move> moves;
  moves.reserve(MAX_SIZE_MOVES_ARRAY);
...
[[maybe_unused]] bool has_legal_moves(Board &board) noexcept
{
...
[[maybe_unused]] std::vector<Move> generate_legal_moves(Board &board) noexcept
{
  std::vector<Move> moves;
  moves.reserve(MAX_SIZE_MOVES_ARRAY);
  ...
  for (const Move &move : movegen::generate_pseudo_legal_moves(board)) {
    ...
    Square king_sq = bitboard::bit_scan_forward(board.get_pieces(board.get_opponent(), KING));
    if (!board.is_square_attacked(king_sq, attacker_side)) { moves.push_back(move); }
    board.unmake(move, state);
  }
  return moves;
}

[[maybe_unused]] std::vector<Move> generate_legal_captures(Board &board) noexcept
{
  ...
  for (const Move &move : movegen::generate_pseudo_legal_captures(board)) {
    ...
    Color attacker_side = board.get_side_to_move();
    Square king_sq = bitboard::bit_scan_forward(board.get_pieces(board.get_opponent(), KING));
    if (!board.is_square_attacked(king_sq, attacker_side)) { captures.push_back(move); }
    board.unmake(move, state);
  }
  return captures;
}
