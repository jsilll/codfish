...
namespace movegen {
    ...
    static void generate_castling_moves(std::vector<Move> &move_list, const Board &board) noexcept {
        ...
        if (!board.IsSquareAttacked(CASTLE_E_SQ, Opponent)) {
            if ((board.castling_availability() & CASTLE_KING_MASK) &&
                !bitboard::get_bit(board.occupancies(BOTH), CASTLE_F_SQ) &&
    ...
    static void generate_slider_moves(std::vector<Move> &move_list, const Board &board) {
        static_assert(PType == BISHOP || PType == ROOK || PType == QUEEN,
                      "Unsupported piece type in generate_slider_moves()");

        constexpr Color Opponent = ToMove == WHITE ? BLACK : WHITE;
        constexpr Bitboard
        (*ATTACKS_FUNC)(const Square sq, Bitboard occ) = (PType == BISHOP ? attacks::get_bishop_attacks : PType == ROOK
