...
constexpr u64 MAGICS_BISHOP[N_SQUARES] = { 0x40040844404084ULL,
  0x2004208a004208ULL,
  0x10190041080202ULL,
...
constexpr u64 MAGICS_ROOK[N_SQUARES] = { 0x8a80104000800020ULL,
...
[[nodiscard]] constexpr u64 mask_bishop_attack_rays(const Square sq) noexcept
{
...
[[nodiscard]] constexpr u64 mask_rook_attack_rays(const Square sq) noexcept
{
  const auto rank = utils::get_rank(sq);
  const auto file = utils::get_file(sq);

  u64 attacks = bitboard::kZERO;
  for (int r = rank + 1; r < RANK_8; ++r) { attacks |= (bitboard::kONE << utils::get_square((Rank)r, file)); }

  for (int r = rank - 1; r > RANK_1; --r) { attacks |= (bitboard::kONE << utils::get_square((Rank)r, file)); }

  for (int f = file + 1; f < FILE_H; ++f) { attacks |= (bitboard::kONE << utils::get_square(rank, (File)f)); }

  for (int f = file - 1; f > FILE_A; --f) { attacks |= (bitboard::kONE << utils::get_square(rank, (File)f)); }
