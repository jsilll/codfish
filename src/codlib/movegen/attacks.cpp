...
[[nodiscard]] constexpr u64 mask_knight_attacks(const u64 knights) noexcept
{
...
u64 get_bishop_attacks(const Square sq, const u64 occ) noexcept
{
  const auto idx = magics::magic_index(occ, magics::MAGIC_TABLE_BISHOP[sq]);
  return ATTACKS_BISHOP[sq][idx];
}

u64 get_rook_attacks(const Square sq, const u64 occ) noexcept
{
