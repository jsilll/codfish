...
namespace bitboard {
    Bitboard SetOccupancy(const int index, const int bits_in_mask, Bitboard attack_mask) noexcept {
        Bitboard occupancy = ZERO;
        for (int bit = 0; bit < bits_in_mask; bit++) {
            Square lsb_sq = BitScanForward(attack_mask);
    ...
    void Display(std::ostream &os, const Bitboard &bb) noexcept {
        for (int i = RANK_8; i >= RANK_1; --i) {
            ...
            for (int n = FILE_A; n < N_FILES; ++n) {
                os << static_cast<int>((bb >> utils::GetSquare((Rank) i, (File) n)) & ONE) << std::string(" ");
            }
            os << "\n";
        }
        os << "\n   a b c d e f g h\n";
